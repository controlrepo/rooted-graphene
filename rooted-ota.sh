...
# renovate: datasource=git-refs packageName=https://github.com/schnatterer/my-avbroot-setup
PATCH_PY_VERSION=a6848660
# TODO renovate
TOMLKIT_VERSION=0.13.2 
PYTHON_VERSION=3.12.9-alpine
...
function patchOTAs() {
  ...
  if ! ls ".tmp/patch.py" >/dev/null 2>&1; then
    curl --fail -sL "https://raw.githubusercontent.com/schnatterer/my-avbroot-setup/${PATCH_PY_VERSION}/patch.py" >.tmp/patch.py
  fi
  ...
  for flavor in "${!POTENTIAL_ASSETS[@]}"; do
    ...
    if ls "$targetFile" >/dev/null 2>&1; then
      ...
      # We need to add .tmp to PATH, but we cant't use $PATH: because this would be the PATH of the host not the container
      ...
      docker run --rm -v $PWD/.tmp:/app/.tmp -w /app \
        ...
        python:${PYTHON_VERSION} sh -c \
          "apk add openssh && \
           pip install tomlkit==${TOMLKIT_VERSION} && \
           python .tmp/patch.py ${args[*]} && \
           chown -R $(id -u):$(id -g) .tmp"
    
       echo "Finished patching file ${targetFile}"
    fi
